...
> **Portfolio evidence status:** PARTIALLY TESTED. Deployed live to AWS on 2026-09-23. A real `full-load-and-cdc` DMS task moved seeded rows from RDS PostgreSQL into the S3 Parquet lake with 0 errors ([evidence](https://github.com/TreyWright360/aws-cloud-operations-handbook/blob/main/evidence/dataops-dms-migration/INDEX.md)); a follow-up pass verified ongoing CDC apply (insert/update/delete captured in ~10s), cross-region replication (~1s measured lag), and a real Glue SCD2 transformation — the original script was an unimplemented stub that would have failed on first run, since replaced with a working implementation and hand-verified output ([evidence](https://github.com/TreyWright360/aws-cloud-operations-handbook/blob/main/evidence/dataops-cdc-replication-scd2/INDEX.md)). Kinesis streaming remains untested — no consumer is wired up to it yet.
